...
export class TestProcessor {
  ...
  private qwenService: QwenService | null
  private layeredModelService: LayeredModelService | null
  ...
  constructor(
    ...
    layeredModelService?: LayeredModelService | null
  ) {
    this.llamaService = llamaService
    this.qwenService = qwenService
    this.layeredModelService = layeredModelService || null
    ...
    // Initialize Intelligent Retry Layer (IRL)
    // Prefer LayeredModelService (Qwen-Coder models) over legacy LlamaService
    const irlModelService = this.layeredModelService || llamaService
    ...
    this.contextSynthesizer = new ContextSynthesizer(llamaService, this.comprehensiveTesting)
  ...
  private async captureDiagnosisSnapshot(params: {
    ...
    const context = await this.llamaService.analyzeScreenshot(
    ...
    const analysis = await this.llamaService.analyzePageTestability(context)
  ...
  private async executeTestSequenceForBrowser(
    ...
    try {
      ...
      if (userInstructions && this.layeredModelService) {
        try {
          console.log(`[${runId}] [${browserType.toUpperCase()}] Using Layered Model Service (Qwen 32B) to parse user instructions: "${userInstructions}"`)
          parsedInstructions = await this.layeredModelService.parseTestInstructions(userInstructions, build.url)
          console.log(`[${runId}] [${browserType.toUpperCase()}] Layered Model Service parsed instructions:`, JSON.stringify(parsedInstructions, null, 2))
        } catch (error: any) {
          console.error(`[${runId}] [${browserType.toUpperCase()}] Layered Model Service parsing failed:`, error.message)
          // Fallback to legacy Qwen service
          if (this.qwenService) {
            try {
              console.log(`[${runId}] [${browserType.toUpperCase()}] Falling back to legacy Qwen service`)
              parsedInstructions = await this.qwenService.parseInstructions(userInstructions, build.url)
            } catch (fallbackError: any) {
              console.error(`[${runId}] [${browserType.toUpperCase()}] Legacy Qwen parsing also failed:`, fallbackError.message)
            }
          }
        }
      } else if (userInstructions && this.qwenService) {
        try {
          console.log(`[${runId}] [${browserType.toUpperCase()}] Using legacy Qwen service to parse user instructions: "${userInstructions}"`)
          parsedInstructions = await this.qwenService.parseInstructions(userInstructions, build.url)
          console.log(`[${runId}] [${browserType.toUpperCase()}] Legacy Qwen parsed instructions:`, JSON.stringify(parsedInstructions, null, 2))
...
`
          ...
          console.error(`[${runId}] [${browserType.toUpperCase()}] Qwen parsing failed:`, error.message)
        ...
        // Qwen not available, use instructions directly
        console.log(`[${runId}] [${browserType.toUpperCase()}] Qwen not available, using instructions directly: ${userInstructions}`)
        if (options?.allPages || options?.testMode === 'all') {
          goal = `USER INSTRUCTIONS (PRIORITY): ${userInstructions}\n\nAdditionally, discover and test all pages on the website starting from ${build.url}. Navigate through all internal links, test each page, and ensure all pages are functional.`
        } else if (options?.testMode === 'multi') {
          goal = `USER INSTRUCTIONS (PRIORITY): ${userInstructions}\n\nAdditionally, navigate through all specified pages and test functionality.`
      ...
      while (stepNumber < maxSteps && Date.now() - startTime < maxDuration) {
        ...
        try {
          ...
          const contextResult = await this.contextSynthesizer.synthesizeContext({
            ...
            appiumRunner: this.appiumRunner,
            stepNumber,
            runId,
            browserType,
          })
          ...
          if (manualAction) {
            ...
            if (isMonkeyMode) {
              ...
              console.log(`[${runId}] [${browserType.toUpperCase()}] Step ${stepNumber}: Generating action with Llama... (${filteredContext.elements.length} unvisited elements available)`)
              action = await this.llamaService.generateAction(
                filteredContext, 
                history, 
                goal,
                {
          ...
          // Optional vision-language validation (checks layout using GPT-4V or compatible VLM)
          let visionIssues: VisionIssue[] = []
          if (
            this.visionValidator &&
            this.visionValidatorInterval > 0 &&
            stepNumber % this.visionValidatorInterval === 0
          ) {
            try {
