...
export abstract class BaseProcessor {
    ...
    /**
     * Record a step with structured output
     * @param action - Step action name
     * @param success - Legacy success boolean (for backwards compatibility)
     * @param duration - Step duration in ms
     * @param details - Extended details including execution_status, observed_state, severity
     */
    protected recordStep(
        ...
        // Extract structured output fields if present
        const execution_status = details?.execution_status
        const observed_state = details?.observed_state
    ...
    protected async captureScreenshot(label?: string): Promise<string | undefined> {
        try {
            ...
            // Use uploadScreenshot which routes to Wasabi when configured
            // Wrap with timeout to prevent BullMQ stall detection from triggering job restarts
            ...
            const timeoutPromise = new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error('Screenshot upload timed out after 30s')), UPLOAD_TIMEOUT_MS)
            )
