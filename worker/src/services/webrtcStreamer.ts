...
export class WebRTCStreamer extends EventEmitter {
  ...
  private async startFrameCapture(page: Page): Promise<void> {
    try {
      ...
      // Start screencast via CDP - OPTIMIZED for streaming performance
      await this.cdpSession.send('Page.startScreencast', {
        format: 'jpeg', // Use JPEG for better performance
        quality: 55,    // Reduced from 80 for faster streaming
        maxWidth: 1280,  // Reduced from 1920 for smaller frames
        maxHeight: 720,  // Reduced from 1080 for smaller frames
        everyNthFrame: 2, // Skip every other frame for smoother streaming
      })
  ...
  private async broadcastFrame(runId: string, frame: Buffer) {
    const now = Date.now()
    // Throttle to ~8 FPS (125ms) to avoid overloading Redis while maintaining smooth appearance
    if (now - this.lastBroadcastTime < 125) {
      return
    }
    ...
    try {
      ...
      // DEBUG: Log every 20th frame to verify broadcast is working
      if (this.frameCount % 20 === 0) {
        const sizeKB = Math.round(payload.state.screenshot.length / 1024)
        console.log(`[WebRTC] Broadcasting frame #${this.frameCount} to runId: ${baseRunId} (Size: ${sizeKB} KB)`)
      }
      ...
      console.error('[WebRTC] Broadcast error:', e.message)
    }
  }
}
