  ...
  async synthesizeContext(params: SynthesizeContextParams): Promise<SynthesizeContextResult> {
    ...
    const filteredElements = context.elements
      .filter((e: VisionElement) => {
        ...
        const className = (typeof e.className === 'string' ? e.className : (e.className?.baseVal || '')).toLowerCase()
      ...
      .map((e: VisionElement) => {
        ...
        const testability = getTestability(e.selector) || undefined
        return {
          ...e,
      ...
      .sort((a, b) => {
        ...
        const aInteractive = !!a.href || a.type === 'button' || a.type === 'link' || a.role === 'button' || a.role === 'link'
        const bInteractive = !!b.href || b.type === 'button' || b.type === 'link' || b.role === 'button' || b.role === 'link'
