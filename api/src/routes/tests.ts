...
export async function testRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    try {
      ...
      if (error) {
        fastify.log.error('Failed to store heuristic:', error)
  ...
  fastify.get<{
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 = not found
        fastify.log.error('Failed to retrieve heuristic:', error)
        return reply.code(500).send({ error: error.message })
      }
  ...
  fastify.post<{
    ...
    try {
      ...
      if (error) {
        fastify.log.error('Failed to find similar components:', error)
        return reply.code(500).send({ error: error.message })
      }
  ...
  fastify.post<{
    ...
    try {
      ...
      if (error) {
        fastify.log.error('Failed to record usage:', error)
        return reply.code(500).send({ error: error.message })
      }
