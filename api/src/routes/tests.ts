...
export async function testRoutes(fastify: FastifyInstance) {
  ...
  // Public access allowed if visibility is 'public', otherwise requires auth
  fastify.get<{ Params: { runId: string } }>('/:runId', {
    preHandler: optionalAuth
  }, async (request: any, reply: any) => {
    try {
      const { runId } = request.params
      const userId = request.userId // From optionalAuth
      ...
      // Access Control
      const isPublic = testRun.visibility === 'public'
      const isOwner = userId && userId === testRun.userId

      // Guest Access Control
      const { getGuestSessionFromCookie } = await import('../lib/urlValidator')
  ...
  // Associate guest test run with user
  fastify.post<{ Params: { runId: string }; Body: { userId: string } }>('/:runId/associate', async (request: any, reply: any) => {
    try {
      const { runId } = request.params
      ...
      // 2. Find a target project for the user
      // Assuming user's personal team ID is their user ID (standard pattern)
      const projects = await Database.listProjects(userId)
      let targetProjectId = projects.length > 0 ? projects[0].id : null

      // 3. Create a project if none exists
      if (!targetProjectId) {
        const newProject = await Database.createProject({
          name: 'My First Project',
          description: 'Created automatically for your first test',
          teamId: userId,
        }, userId)
        targetProjectId = newProject.id
      }
      ...
      const updatedRun = await Database.updateTestRun(runId, {
        userId,
        projectId: targetProjectId,
        guestSessionId: null as any, // Clear guest session ID to convert to regular test
      })

      fastify.log.info({ runId, userId, projectId: targetProjectId }, 'Associated guest test with user')

      return reply.send({ success: true, testRun: updatedRun })
    } catch (error: any) {
      fastify.log.error(error)
      return reply.code(500).send({ error: error.message || 'Failed to associate test run' })
    }
  })
}
