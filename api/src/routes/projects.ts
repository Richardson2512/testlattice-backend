...
export async function projectRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: { name: string; description?: string; teamId: string } }>('/', {
    ...
    try {
      const { name, description, teamId } = request.body

      if (!name || !teamId) {
        return reply.code(400).send({ error: 'Name and teamId are required' })
      }

      fastify.log.info('Creating project:', { name, teamId, userId: request.user?.id })
      ...
      fastify.log.info('Project created successfully:', project.id)
