    ...
    this.redisSub.on('message', (channel, message) => {
      if (channel === 'ws:broadcast') {
        try {
          ...
          // console.log(`[Redis] Received ${message.length} bytes on channel ${channel}`)
          ...
          if (data.payload?.type) {
            ...
            console.log(`[Redis] RX: ${data.payload.type} | RunId: ${data.runId} | Size: ${sizeKB}KB | Server: ${data.serverId}`)
          }

          // Only broadcast to local connections (avoid echo)
          if (data.serverId !== this.serverId) {
            // DEBUG: Log page_state messages specifically
            if (data.payload?.type === 'page_state') {
              const connections = this.connections.get(data.runId)
              console.log(`[WS] Relaying page_state to ${connections?.length || 0} clients`)
            }
